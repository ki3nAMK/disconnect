...
import { _checkFileExists } from 'src/helpers/filter-file-upload.helper';
...
export class IsThumbnailPathConstraint implements ValidatorConstraintInterface {
	validate(value: string) {
		return (
			value.startsWith('assets\\uploads\\') ||
			value.startsWith('assets/uploads/') ||
			value.startsWith('assets\\defaults\\') ||
			value.startsWith('assets/defaults/')
		);
	}
...
export class IsThumbnailExistsConstraint
